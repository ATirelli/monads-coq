...
(** Implement constant propagation for Imp expressions and statements *)
...
(*Fixpoint constant_propagation_expr (e : expr) (s : env) : expr :=
  match e with
  | Var x => if (Set.member x s) then e else Lit 0
  | Lit n => Lit n
  | Plus a b => Plus (constant_propagation_expr a s) (constant_propagation_expr b s)
  | Minus a b => Minus (constant_propagation_expr a s) (constant_propagation_expr b s)
  | Mult a b => Mult (constant_propagation_expr a s) (constant_propagation_expr b s)
  end.

Fixpoint constant_propagation_stmt (s : stmt) (env : list (var * value)) : stmt :=
  match s with
  | Assign x e => Assign x (constant_propagation_expr e (map fst env))
  | Seq s1 s2 => Seq (constant_propagation_stmt s1 env) (constant_propagation_stmt s2 env)
  | If e s1 s2 => If (constant_propagation_expr e (map fst env)) (constant_propagation_stmt s1 env) (constant_propagation_stmt s2 env)
  | While e s => While (constant_propagation_expr e (map fst env)) (constant_propagation_stmt s env)
  | Skip => Skip
  end.
*)
