...
(* packaging the monadic laws-- looks like we have to do it one at a time*)
...
(* with proof term *)
Instance ReadMonadLeft : MonadLeft :=
  {
left_id_monad := fun (A B : Type) (x : A) (f : A -> read_comp B) =>
functional_extensionality (fun _ : state => eq_refl)
  }.

(* tactics *)
Instance ReadMonadRight : MonadRight.
constructor. intros. apply functional_extensionality.  trivial.
Defined.

Instance ReadMonarAssoc: MonadAssoc.
constructor. intros. apply functional_extensionality.  trivial.
Defined.
